...
function initApp() {
  ...
  function renderCreatorsList() {
    ...
    pageItems.forEach(c => {
      ...
      const avatarHTML = c.avatar
        ? `<div class="avatar" style="background:transparent"><img src="${c.avatar}" alt="${c.name}" style="width:100%;height:100%;object-fit:cover;border-radius:50%;display:block" /></div>`
        : `<div class="avatar" style="background:rgba(52,224,204,0.12);color:var(--teal);display:grid;place-items:center;font-weight:700;font-size:16px;flex-shrink:0">${initials}</div>`;
  ...
  async function handleConnectClick(creatorId) {
    ...
    // Safeguard DOM elements to prevent crashes on dashboard/admin page
    const msgTitle = document.getElementById('msgTitle');
    const msgSubject = document.getElementById('msgSubject');
    const msgBody = document.getElementById('msgBody');
    
    if (msgTitle) msgTitle.textContent = `Message ${creator.name}`;
  ...
  function openProfileDetails(creatorId) {
    ...
    const detailAvatarHTML = creator.avatar
      ? `<div class="profile-avatar" style="background:transparent"><img src="${creator.avatar}" alt="${creator.name}" style="width:100%;height:100%;object-fit:cover;border-radius:50%;display:block" /></div>`
      : `<div class="profile-avatar" style="font-size:26px;background:rgba(52,224,204,0.12);color:var(--teal);display:grid;place-items:center;font-weight:700;flex-shrink:0">${initials}</div>`;
    ...
    if (showWaButton) {
      ...
      waBtn.addEventListener('click', () => {
        ...
        // 2. Open WhatsApp immediately and synchronously (no popup block!)
        window.open(waUrl, '_blank');

        // 3. Log the connection to database in the background (fire-and-forget)
        const supabase = window.supabaseClient;
        if (supabase) {
          supabase
            .from('messages')
